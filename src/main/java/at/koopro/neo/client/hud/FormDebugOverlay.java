 ...
 * Toggled via {@code /neo wizmorph debug <player> on|off}.
 */
public final class FormDebugOverlay {

    public static final Identifier ID =
            Identifier.fromNamespaceAndPath(Neo.MODID, "form_debug");
    ...
    public static void render(GuiGraphics graphics, DeltaTracker delta) {
        if (!ClientFormDataHolder.isDebugOverlay()) return;

        Minecraft mc = Minecraft.getInstance();
        if (mc.player == null) return;

        ClientFormDataHolder.FormData data = ClientFormDataHolder.get(mc.player.getUUID());
