...
public class PortkeyItem extends Item {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display,
        ...
        if (target != null) {
            tooltipAdder.accept(Component.translatable("item.neo.portkey.linked",
                            target.getX(), target.getY(), target.getZ())
                    .withStyle(ChatFormatting.DARK_AQUA));
        } else {
            tooltipAdder.accept(Component.translatable("item.neo.portkey.unlinked").withStyle(ChatFormatting.GRAY));
        }
    }
}
