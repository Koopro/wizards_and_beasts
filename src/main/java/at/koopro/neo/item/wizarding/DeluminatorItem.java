...
import at.koopro.neo.client.deluminator.DeluminatorRenderer;
...
public class DeluminatorItem extends GeoItemBase {
    ...
    private static boolean isOpen(ItemStack stack) {
        ...
        return data.copyTag().getBoolean("Open").orElse(false);
    }
    ...
    static int getStoredLights(ItemStack stack) {
        return stack.getOrDefault(ModDataComponents.STORED_LIGHTS.get(), 0);
    }

    static void adjustStoredLights(ItemStack stack, int delta) {
        int current = getStoredLights(stack);
        stack.set(ModDataComponents.STORED_LIGHTS.get(), Math.max(0, current + delta));
    }
}
