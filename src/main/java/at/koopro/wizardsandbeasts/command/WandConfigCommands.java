...
/**
 * Registers the /wandb wand config &lt;subcommand&gt; tree.
...
public final class WandConfigCommands {
    ...
    // ── /wandb wand config set <slot> <module_id> ───────────────────────────
    ...
    private static int executeSet(ServerPlayer player, String slotArg, String moduleArg) {
        ...
        player.displayClientMessage(
                success("Set " + slot.slotId() + " → " + moduleId), false);
        return 1;
    }

    // ── /wandb wand config clear <slot> ─────────────────────────────────────
