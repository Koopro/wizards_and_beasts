...
public final class TypeSelectionRenderHelper {
    ...
    public static final int COL_BG = WizardsAndBeastsUiTokens.TypeSelection.COLOR_BG;
    public static final int COL_BORDER_LIGHT = WizardsAndBeastsUiTokens.TypeSelection.COLOR_BORDER_LIGHT;
    public static final int COL_BORDER_DARK = WizardsAndBeastsUiTokens.TypeSelection.COLOR_BORDER_DARK;
    public static final int COL_INNER_BG = WizardsAndBeastsUiTokens.TypeSelection.COLOR_INNER_BG;
    public static final int COL_CARD_BG = WizardsAndBeastsUiTokens.TypeSelection.COLOR_CARD_BG;
    public static final int COL_CYAN = WizardsAndBeastsUiTokens.TypeSelection.COLOR_CYAN;
    public static final int COL_TEXT = WizardsAndBeastsUiTokens.TypeSelection.COLOR_TEXT;
    public static final int COL_DIM = WizardsAndBeastsUiTokens.TypeSelection.COLOR_DIM;
    public static final int COL_GREEN = WizardsAndBeastsUiTokens.TypeSelection.COLOR_GREEN;
    public static final int COL_RED = WizardsAndBeastsUiTokens.TypeSelection.COLOR_RED;
    public static final int COL_INK = WizardsAndBeastsUiTokens.TypeSelection.COLOR_INK;
    public static final int COL_ACCENT = WizardsAndBeastsUiTokens.TypeSelection.COLOR_ACCENT;
    public static final int COL_DIVIDER = WizardsAndBeastsUiTokens.TypeSelection.COLOR_DIVIDER;
    public static final int COL_PARCHMENT_BRIGHT = WizardsAndBeastsUiTokens.TypeSelection.COLOR_PARCHMENT_BRIGHT;
    public static final int COL_PARCHMENT_DARK = WizardsAndBeastsUiTokens.TypeSelection.COLOR_PARCHMENT_DARK;
    ...
    public static void renderTypeCard(GuiGraphics g, Font font, int px, int py, int panelW, int panelH,
                                      @Nullable WizType selectedType, ScreenLayoutScaler layout) {
        WizType resolved = selectedType == null ? WizType.values()[0] : selectedType;

        drawFrame(g, px, py, panelW, panelH);

        int cardX = px + layout.s(WizardsAndBeastsUiTokens.TypeSelection.CARD_X);
        int cardY = py + layout.s(WizardsAndBeastsUiTokens.TypeSelection.CARD_Y);
        int cardW = layout.s(WizardsAndBeastsUiTokens.TypeSelection.CARD_W);
        int cardH = layout.s(WizardsAndBeastsUiTokens.TypeSelection.CARD_H);
        ...
        int headerX = px + layout.s(WizardsAndBeastsUiTokens.TypeSelection.HEADER_X);
        int headerY = py + layout.s(WizardsAndBeastsUiTokens.TypeSelection.HEADER_Y);
        int headerW = layout.s(WizardsAndBeastsUiTokens.TypeSelection.HEADER_W);
        int headerH = layout.s(WizardsAndBeastsUiTokens.TypeSelection.HEADER_H);
        ...
        int iconX = px + layout.s(WizardsAndBeastsUiTokens.TypeSelection.TYPE_ICON_X);
        int iconY = py + layout.s(WizardsAndBeastsUiTokens.TypeSelection.TYPE_ICON_Y);
        drawTypeIcon(g, resolved, iconX, iconY, layout);
        int headerTextX = iconX + layout.s(WizardsAndBeastsUiTokens.TypeSelection.TYPE_ICON_SIZE)
                + layout.s(WizardsAndBeastsUiTokens.TypeSelection.HEADER_TEXT_GAP);
        g.drawString(font, normalizeName(resolved.getDisplayName()),
                headerTextX, py + layout.s(WizardsAndBeastsUiTokens.TypeSelection.TITLE_Y), COL_INK, false);
        List<FormattedCharSequence> descLines = font.split(Component.literal(resolved.getDescription()),
                layout.s(WizardsAndBeastsUiTokens.TypeSelection.HEADER_W - WizardsAndBeastsUiTokens.TypeSelection.TYPE_ICON_SIZE - 20));
        int descY = py + layout.s(WizardsAndBeastsUiTokens.TypeSelection.HEADER_DESC_Y);
        for (int i = 0; i < Math.min(WizardsAndBeastsUiTokens.TypeSelection.HEADER_DESC_MAX_LINES, descLines.size()); i++) {
            g.drawString(font, descLines.get(i), headerTextX, descY, COL_DIM);
            descY += font.lineHeight;
        }

        int textX = px + layout.s(WizardsAndBeastsUiTokens.TypeSelection.BODY_X);
        int y = py + layout.s(WizardsAndBeastsUiTokens.TypeSelection.BODY_START_Y);
        int lineH = layout.s(WizardsAndBeastsUiTokens.TypeSelection.LINE_H);
        ...
        g.fill(textX, y - 4, px + panelW - layout.s(WizardsAndBeastsUiTokens.TypeSelection.INFO_RIGHT_PAD), y - 3, COL_DIVIDER);
    ...
    public static void renderSubtypeList(GuiGraphics g, Font font, int px, int py, int panelW,
        ...
        int iconX = px + layout.s(WizardsAndBeastsUiTokens.TypeSelection.SUBTYPE_ICON_X);
        int iconY = py + layout.s(WizardsAndBeastsUiTokens.TypeSelection.SUBTYPE_ICON_Y);
        drawTypeIcon(g, selectedType, iconX, iconY, layout);

        g.drawString(font, normalizeName(selectedType.getDisplayName()), iconX + layout.s(WizardsAndBeastsUiTokens.TypeSelection.TYPE_ICON_SIZE + 10), py + layout.s(20), COL_INK, false);
        g.fill(px + layout.s(WizardsAndBeastsUiTokens.TypeSelection.DIVIDER_LEFT), py + layout.s(WizardsAndBeastsUiTokens.TypeSelection.DIVIDER_Y),
                px + panelW - layout.s(WizardsAndBeastsUiTokens.TypeSelection.DIVIDER_RIGHT), py + layout.s(WizardsAndBeastsUiTokens.TypeSelection.DIVIDER_Y) + 1,
                COL_DIVIDER);

        int infoY = py + layout.s(34);
        int subtypeHeaderTextX = iconX + layout.s(WizardsAndBeastsUiTokens.TypeSelection.TYPE_ICON_SIZE + 10);
        List<FormattedCharSequence> subtypeDesc = font.split(Component.literal(selectedType.getDescription()),
                layout.s(WizardsAndBeastsUiTokens.TypeSelection.HEADER_W - WizardsAndBeastsUiTokens.TypeSelection.TYPE_ICON_SIZE - 22));
        ...
        for (int i = 0; i < subtypes.size(); i++) {
            ...
            int y = listY + i * (layout.s(WizardsAndBeastsUiTokens.TypeSelection.LIST_BUTTON_HEIGHT) + layout.s(WizardsAndBeastsUiTokens.TypeSelection.LIST_BUTTON_GAP)) + layout.s(6);
    ...
    public static void renderConfirmation(GuiGraphics g, Font font, int px, int py, int panelW, int panelH,
                                          @Nullable WizType selectedType, @Nullable WizSubtype selectedSubtype, ScreenLayoutScaler layout) {
        ...
        int iconX = px + layout.s(WizardsAndBeastsUiTokens.TypeSelection.CONFIRM_ICON_X);
        int iconY = py + layout.s(WizardsAndBeastsUiTokens.TypeSelection.CONFIRM_ICON_Y);
        drawTypeIcon(g, selectedType, iconX, iconY, layout);
        ...
        int y = py + layout.s(WizardsAndBeastsUiTokens.TypeSelection.DIVIDER_Y) + layout.s(8);
        int x = px + layout.s(18);

        g.drawString(font, normalizeName(selectedType.getDisplayName()) + " - " + normalizeName(selectedSubtype.getDisplayName()),
        ...
        List<FormattedCharSequence> typeDescLines = font.split(Component.literal(selectedType.getDescription()),
                layout.s(WizardsAndBeastsUiTokens.TypeSelection.CARD_W - 28));
    ...
    private static Identifier resolveTypeIcon(WizType type) {
    ...
    private static void drawTypeIcon(GuiGraphics g, WizType type, int x, int y, ScreenLayoutScaler layout) {
        int size = layout.s(WizardsAndBeastsUiTokens.TypeSelection.TYPE_ICON_SIZE);
