...
The actual play loops:
	...
	•	First hour: Acquire wand → find a written spellbook (village shelves, stronghold libraries, a hidden wizarding cache) → read it to learn what is written in it (`SpellSourceItem`) → cast to build proficiency. Nobody sells spells. The spell-teacher vendor and its Knuts price were removed on 2026-09-10; a blank Standard Book of Spells plus an ink bottle at a Study Lectern (`SpellScriptoriumBlock`, still registered `spell_teacher`) lets a wizard who knows a spell write a copy for someone else, which is how a spell spreads on a server after the first copy is found.
...
Currency — one sink lighter, and the remaining one is honest. The Gringotts vault (Knuts/Sickles/Galleons) gated both wand-buying and spell-learning; the spell half is gone with the vendor (2026-09-10), because charging for the contents of your own head is what made the early game read as a shop rather than a school. Money now buys objects only — a wand, a broom, a book someone else wrote. That leaves wand-buying and the skill respec fee as the sinks, which is thinner than before: **the currency system now needs a second real sink that is not a spell wall.** Open.
