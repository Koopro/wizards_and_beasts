...
│   ├── map, owl, skill, spell, stats, trinket, trunk, wand
├── owl (command, data)
├── particle
├── registry                               NeoForge registries and grouped item/block registrars
├── skill (command, data, vocation)
├── spell
│   ├── beam, cast, command, core, data, debug, def, effect, expelliarmus, gamp,
│   ├── imperio, impl, learning, lib, patronus, petrify, proficiency, protego, study
├── stats
...
|---|---|---|
| Wand | `WandItem`, `WandCast*`, `WandComponents`, `WandAttachments`, allegiance/corruption/integrity/resonance packages | Stack components hold wood, core, flexibility, master, bond, bond history, integrity, corruption and config — a wand's allegiance lives on the stack, never on a player; only the Elder Wand's master is also held in `ElderWandSavedData`, which wins over the stack. `WandAllegianceRules` (pure) decides states, power, transfers and backfires; `WandAllegianceService` applies them. Wood/core `temperament` blocks tune the rules per datapack. Extensible through datapack woods, cores, bench enhancers and `WandModule`. |
...
| Nutrition / blood | `heritage.nutrition.*` (`NutritionPolicy`, `NutritionPolicyResolver`, `NutritionEnforcer`), `heritage.vampire.*`, `VAMPIRE_BLOOD` attachment | A heritage variant resolves to a nutrition policy; `blood_hunger` means BLOOD. The vanilla food level is *driven* from the blood pool rather than hidden, so vanilla's sprint, regeneration and starvation rules keep working off one number. Empty-hand entity interaction feeds; food nutrition is snapshotted and restored. `BloodDataSyncS2CPayload` carries the policy and the server-decided thirst band; `BloodBarRenderer` is `Gui.renderFood`'s loop with different sprites, replacing the cancelled `food_level` layer. Icons are generated by `tools/hud_blood_icons.py` into `textures/gui/sprites/hud/` and resolved as `wizards_and_beasts:hud/blood_*` off the vanilla `minecraft:gui` atlas — its `directory` source walks every namespace, so no atlas file of ours is needed. |
...
| Bestiary | `BestiaryEntryLoader`, `PlayerBestiaryData`, `BestiaryDiscoveryHandler`, `EncounterRule`, `CreatureProfile` | 107 JSON entries, each with a `profile` (classification, diet, behaviour, interactions, materials, society, study acts, signature, canon basis). Tiers `UNKNOWN → ENCOUNTERED → OBSERVED → STUDIED → KNOWN` are earned by sighting, calm watching (`observation` ticks on the attachment), study acts and signature behaviour — never by killing. Species code reports through `BestiaryDiscoveryHandler.encountered/studied/witnessedSignature`. |
| Wildlife behaviour | `creature/wildlife` (`WildlifeRules` pure, `WildlifeWorld`, `CreatureSign`, `LycanthropyInfection`), abilities `shed`, `wary`, `watched_invisibility`, `foresight`, `groomable`, `moon_bound`, `lycanthropic_bite`, `slayer_curse` | Materials come from living creatures: `CreatureSign` marks shed items by species (pickup = study); `groomable` gives on trust. Bespoke: `PhoenixEntity` rebirth + bond loyalty, `MooncalfEntity` burrow/gather/dance, `BowtruckleEntity` home tree + defence. |
...
| Ministry law | `ministry.law.*` (`TraceService`, `MagicalOffence`, fines), `ministry.trace.*` (`TraceRules`, `CaseRules`, `Wizengamot` pure; `MinistryTrace` service; `CastSurvey`) | `TraceService.report` stays the one seam that files an offence. A successful cast calls `MinistryTrace.onSuccessfulCast`, which surveys witnesses and stores an `Incident` plus delayed `PendingReport`s in `MinistryCaseData` (overworld `SavedData`: incidents, reports, per-UUID `Dossier`, wand residue). `MinistryTraceEvents` runs `process(server, now)` once a second; everything is absolute overworld game time, so death, logout, dimension change and restart change nothing. Spell law is datapack `spell_law/`; Muggles are the `muggles` entity tag. Age and wand confiscation live on `PlayerMinistryRecord`. |
...
- Use targeted sync/deltas after a mutation, and lifecycle sync on login, respawn and dimension change. Feature lifecycle handlers are part of that contract.
...
| `./gradlew runGameTestServer` | Headless NeoForge server; runs the registered game tests and exits. As of 2026-09-17 that is 84 scenarios; the wand cast/release path is covered by `gametest/WandCastLifecycleTests`, `WandCastRaceTests` and `AfterDeathInputTests`, so a green run proves both that the server boots and that those scenarios pass. Everything outside the cast path still has no in-world coverage — see `KNOWN_ISSUES.md` §8. **1.21.11 has no `@GameTest` annotation**: tests are entries in the `TEST_INSTANCE` registry, added via NeoForge's `RegisterGameTestsEvent` on the mod bus. |
...
| `at.koopro.wizardsandbeasts.spell.learning.SpellLearningService` | Learning eligibility + the one write | Spells |
| `at.koopro.wizardsandbeasts.spell.learning.SpellSource` | `spell_source` component read/write, registry-free naming | Spells |
| `at.koopro.wizardsandbeasts.item.spell.SpellSourceItem` | Reading a book teaches its spell | Spells |
| `at.koopro.wizardsandbeasts.spell.study.SpellScriptoriumBlock` | Study Lectern — scribes a known spell into a blank book (id: `spell_teacher`) | Spells |
| `at.koopro.wizardsandbeasts.loot.SpellbookLootModifier` | Rolls a written book into world loot | Spells |
...
4. Add skill unlock/proficiency data as appropriate, assets/lang, tests for codecs/rules, and a packet only if no existing cast/sync flow covers it.
   - **Nothing sells spells.** If the spell should be findable, add its id to a `spellbook_*` loot-modifier pool (`data/.../loot_modifiers/`); `SpellbookLootPoolTest` checks the id resolves and is neither a Dark Art nor an Obscurial ability. Otherwise it is reached through a skill node's `learn_spell` effect, or scribed by a player who already knows it.
   - Gates go in `SpellLearningEligibility` and nowhere else. Every learning path asks it.
...
- The wand release is one token per server-observed hold (`WandCastSessions` + `CastReleaseGate`); rapid, duplicated, late and cross-player releases are covered by `WandCastRaceTests`. What no test covers is a real network — see `KNOWN_ISSUES.md` §8. Interrupted beam channels still cost no cooldown (§8c.2).
...
- Rapid and duplicated wand releases resolve to at most one cast per hold (verified in game tests on mock connections, not over a real network). An interrupted beam channel costs no cooldown — `KNOWN_ISSUES.md` §8c.2.
...
- `runServer` can fail on some local environments with `FatalStartupException: Couldn't find Minecraft server thread` during NeoForge launch bootstrap; `compileJava/test` and `runGameTestServer` remain the current CI-style verification path while this startup issue is investigated. Note: `runGameTestServer` runs four in-world cast-lifecycle scenarios as of 2026-09-03; every other system still has no in-world coverage (see `KNOWN_ISSUES.md` §8).
...
- **Obscurial abilities (dark form only):** Separate from spell slots; `SpellLearningEligibility` refuses them outright, so no spell source can teach one and no loot pool may list one, and they never appear in slot assignment. Enter dark form, then `N` = `Obscurus Surge`, `M` = `Obscurus Grasp`. Verify cooldown/readiness in Obscurus HUD panel and action-bar feedback for lockout/cooldown/invalid-form.
...
| Layer | Type | What it is | Numbers it may carry |
|---|---|---|---|
| Heritage | `Heritage` | the people a character belongs to | only where canon is physical (giant size/hide, centaur frame, small goblins and elves) |
| Lineage | `HeritageVariant` | the family, culture or circumstance inside that people | none, except the half-giant's smaller body |
| Condition | `MagicalCondition` + `ConditionOrigin` | something that happened to them: lycanthropy, an Obscurus | none |

Rules that follow from it:

- **Ask the character, not the enum.** `PlayerHeritageData.hasTrait(id)` is the union of the lineage's tags and
  the condition's, and `canUseWand()` / `canCast()` are the two capability questions. A check written as
  `getSelectedHeritage() == Heritage.X` cannot see a condition and will silently stop holding.
- **POWER lives in `PowerBandTable`**, which now has exactly two bands: everyone with magic, and the Squib.
- **`HeritageAPI.commit`** is the one routine that takes a heritage on (roll, lock, body, attributes, sync).
  **`afflict`** adds a condition without re-rolling or re-locking anything; **`cure`** removes it; **`clear`**
  takes the whole heritage back off.
- **Forms**: `HeritageFormBridge.getDefaultFormId(heritage, variant, condition, state)` — a condition decides
  the body when there is one, because a condition is what changes the body.
- **Traits are displayed from `HeritageTraits`**, which maps a trait id to a name key, a description key and a
  heading (`TRAIT` / `AFFINITY` / `CHARACTERISTIC`). Declaring a tag on a lineage without adding it here makes
  it invisible to players; `HeritageTraitCatalogTest` fails both ways round, including on a missing lang key.
- **A condition is never broadcast.** `HeritageDataSyncS2CPayload` carries it to its own player only; other
  clients learn the form, never the condition. A `heritage_appearance` entry may name a `condition` instead of
  a `heritage`, and those resolve through `HeritageAppearanceRegistry.resolveCondition` server-side.
- **Save format**: `PlayerHeritageData` version 2 keys `Condition` and `ConditionOrigin`; the migrator turns a
  pre-2 werewolf/obscurial heritage into Wizardkind + condition. Adding a condition or changing an id means a
  `CURRENT_VERSION` bump and a migrator arm.
