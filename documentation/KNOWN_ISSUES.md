...
### 4.2 Placeholder rigs — none remain

Until 2026-09-16, 82 creatures (80 definitions carrying `"_comment": "PLACEHOLDER box rig …"`, plus
the bespoke `hidebehind` and `runespoor`) were drawn as six to nine flat-shaded boxes from
`tools/creature_gen.py`. All 82 were rebuilt in five waves, each creature from its own
`tools/<id>_model.py`:

| Wave | Built on | Creatures |
|---|---|---|
| 1 | `tools/rigkit.py` | unicorn, centaur, kneazle, crup, nundu, griffin, manticore, demiguise, acromantula, troll, puffskein, pygmy_puff |
| 2 | `bodies.quadruped`, `tools/lizard.py` | erumpent, graphorn, chimaera, sphinx, qilin, reem, tebo, zouwu, wampus_cat, matagot, jarvey, knarl, moke, salamander, murtlap, nogtail, dugbog, hodag |
| 3 | `bodies.biped` | clabbert, erkling, gnome, imp, leprechaun, maledictus, porlock, pukwudgie, red_cap, rougarou, giant, yeti, hidebehind |
| 4 | `bodies.bird`, `wing_pair`, `tools/winged_horse.py`, `tools/insect.py` | abraxan, aethonan, granian, snallygaster, thunderbird, fwooper, golden_snidget, jobberknoll, diricawl, billywig, doxy, fairy, glumbumble, swooping_evil |
| 5 | `bodies.serpent`, `splay_leg`, `sphere`, `tools/fish.py`, `tools/crab.py` | grindylow, hippocampus, kappa, kelpie (+ `kelpie_disguise`), merperson, plimpy, ramora, shrake, ashwinder, horned_serpent, sea_serpent, runespoor, boggart, bundimun, lethifold, lobalug, pogrebin, toad, flobberworm, horklump, blast_ended_skrewt, chizpurfle, fire_crab, mackled_malaclaw, quintaped |

`rigkit.emit` refuses fractional cube sizes, animated bones the rig does not have, one-shot clips
nothing can trigger (only `attack/strike/bite/lunge`, `hit/flinch`, `groan/hiss/howl/call/song` and
`gaze` have callers), a declared sheet that differs from the skin, and any texel `uv_check` finds
unpainted; it writes the glowmask from the final skin or deletes a stale one. `tools/` is gitignored,
so the generators live on disk only; the first run of one over a skin another tool wrote needs
`--force`, after which it re-runs byte-identically.

Changes outside the art:

- **The Unicorn's hitbox is 1.4 x 1.6**, a vanilla horse's, up from the 0.95 x 1.25 bulk bucket a
  horse drawn at true proportion does not fit.
- **Two player forms were refitted to their new rigs:** `centaur_default` (`modelScale` 1.570 ->
  1.040) and `merpeople_water` (0.9 -> 0.997), both with aspect 1.0. `FormRigScaleTest` now covers
  the merfolk form too.
- **Stale glowmasks were the one shipped defect.** `GeoRendererHelper.applyGlowIfPresent` lights a
  creature whenever `<id>_glowmask.png` exists, sampled through the new UVs; wave 1 left six old
  masks in place. `GlowmaskParityTest` now requires every mask to match its skin.
- **The Kelpie no longer glows at all.** GeckoLib derives the emissive texture from the current one,
  so a glowing Kelpie in disguise looked for a `kelpie_disguise_glowmask.png` that never existed.
- **`RigMarkerConsistencyTest` is retired**, as its javadoc asked once the backlog was empty.
  `CreatureRigGuardTest` replaces it: no placeholder marker anywhere (`creature_gen.py` re-emits it),
  and every definition's rig, animation and skin exist and agree.

**Still true in a default install:** spawning and the creative tab are decided by
`AlphaRoster.SHIPPED`, not by rig quality, and that list was not changed. So every creature rebuilt
here still does not spawn naturally (`creatureNaturalSpawns = ALPHA_ONLY`) and its egg is hidden
(`showPlaceholderSpawnEggs = false`) — except the Unicorn, which is exempt from the spawn gate as the
only wild source of unicorn hair. `/wandb beast creature summon <id>` reaches all of them. Opening the
roster is a separate decision; none of the rebuilt rigs has been checked in a running client yet.
