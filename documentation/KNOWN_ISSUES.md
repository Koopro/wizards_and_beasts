...
**Wave 1 (2026-09-16) took twelve off the list:** `unicorn`, `centaur`, `kneazle`, `crup`, `nundu`,
`griffin`, `manticore`, `demiguise`, `acromantula`, `troll`, `puffskein`, `pygmy_puff` — the
creatures a player meets first, and one of every ground body plan. Each is generated by its own
`tools/<id>_model.py` on a shared kit, `tools/rigkit.py`, which refuses fractional cube sizes,
animated bones the rig does not have, and one-shot clips nothing can trigger (only
`attack/strike/bite/lunge`, `hit/flinch`, `groan/hiss/howl/call/song` and `gaze` have callers).
Two consequences outside the art:

- **The Unicorn's hitbox is now 1.4 x 1.6**, a vanilla horse's, up from the 0.95 x 1.25 bulk bucket.
  Drawn at true proportion a horse does not fit that box; the old one ended at its shoulder.
- **`RigMarkerConsistencyTest` now checks the whole generator signature**, including one cube per
  non-root bone. The Puffskein and Pygmy Puff are seven cubes because a ball needs no more, and
  root-plus-count alone called them box rigs.

**Wave 2 (2026-09-16), quadrupeds on the shared `tools/bodies.py` archetype:** `erumpent`,
`graphorn`, `chimaera`, `sphinx`, `qilin`, `reem`, `tebo`, `zouwu`, `wampus_cat`, `matagot`,
`jarvey`, `knarl`, `moke`, `salamander`, `murtlap`, `nogtail`, `dugbog`, `hodag`.
...
**This list is the art backlog.** These 52 are what still needs a modeller; nothing else does.

`abraxan`, `aethonan`, `ashwinder`, `billywig`, `blast_ended_skrewt`, `boggart`
`bundimun`, `chizpurfle`, `clabbert`, `diricawl`, `doxy`, `erkling`
`fairy`, `fire_crab`, `flobberworm`, `fwooper`, `giant`, `glumbumble`
`gnome`, `golden_snidget`, `granian`, `grindylow`, `hidebehind`, `hippocampus`
`horklump`, `horned_serpent`, `imp`, `jobberknoll`, `kappa`, `kelpie`
`leprechaun`, `lethifold`, `lobalug`, `mackled_malaclaw`, `maledictus`, `merperson`
`plimpy`, `pogrebin`, `porlock`, `pukwudgie`, `quintaped`, `ramora`
`red_cap`, `rougarou`, `runespoor`, `sea_serpent`, `shrake`, `snallygaster`
`swooping_evil`, `thunderbird`, `toad`, `yeti`
...
The **Unicorn is exempt from the spawn gate**: it is the only wild source of unicorn hair, one of
the three wand cores, and gating it would dead-end wandmaking. It used to be the one placeholder
rig a default install showed; since wave 1 it is a finished rig, and the exemption is unchanged.
